goApp.setPageTitle('GTK::Accounts');

/**
 * App-specific strings
 */
goApp.addStrings({
	confirmDelete: 'Delete selected user account?',
	create: 'Create',
	createAccount: 'Create new user account',
	editAccount: 'User account properties',
	modifiedLost: 'Several account that you modify were removed by '
...
goApp.addErrorMessage(goApp.errors.ERR_RECORD_LOST,
	'User account that you edit was removed by third party application.');
	...
	datatype: function(oParams)
	{
		...
		var
			...
			oCommand	= {
				...
				needTotalSize: true,
				needFields: !goApp.validator,
				fields: ['id', 'name', 'organization_id', 'status', 'password', 'created', 'active', 'email', 'login'],
				page: {
		...
		oRequest.add(oCommand);
		setTimeout(function(){oRequest.send(goApp.processSelectResponse, goApp.processFailedRequest)}, 500);
	},
	jsonReader: {
		repeatitems: false
	},
...
goApp.showCreateDialog	= function()
{
	...
	// clear fields
	$('#name,#organization_id,#email,#status,#password,#login').val('');
...
goApp.createOrganization = function()
{
	...
	// send request
	var oRequest	= new Request('/json/accounts');
	oRequest.add({
		type: 'create',
		data: oData
	});
