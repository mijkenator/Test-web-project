goApp.setPageTitle('GTK::Organization admins');

/**
 * App-specific strings
 */
goApp.addStrings({
	confirmDelete: 'Delete selected organization admin?',
	create: 'Create',
	createAccount: 'Create new organization admin',
	editAccount: 'Organization admin properties',
	modifiedLost: 'Several organization admins that you modify were removed by '
...
goApp.addErrorMessage(goApp.errors.ERR_RECORD_LOST,
	'Organization admin that you edit was removed by third party application.');

/**
 * ID of current account
 */
goApp.currentAccountId	= null;
...
goApp.get_reseller_name_by_id = function(sValue)
{
	return goApp.ResellerData[sValue]
};


goApp.get_resellers = function()
{
	var
	...
	oRequest.send(function(O){
		...
		var options = $('#reseller_id').attr('options');
		for(var i=0;i < oResult.data.length;i++){
			goApp.ResellerData[oResult.data[i].id] = oResult.data[i].name;
		}
	}, function(O){});
};
...
goApp.createGrid('grid', {
	...
	colModel: [
		...
		},
		{
			...
			width: 190,
			classes: 'bold',
			formatter: goApp.get_reseller_name_by_id
		},
		{
			name: 'email',
			index: 'email',
	...
	datatype: function(oParams)
	{
		...
		var
			oRequest	= new Request('/json/orgadmin'),
			oCommand	= {
				type: 'select',
				needTotalSize: true,
				needFields: !goApp.validator,
				fields: ['id', 'name', 'reseller_id', 'organization_id', 'email', 'info', 'password', 'created', 'active'],
				page: {
...
$('#menu-item-devices').click(function() {
	location	= goApp.path + 'contacts/' + goApp.currentAccountId;
});
$('#menu-item-call-history').click(function() {
	location	= goApp.path + 'call-history/' + goApp.currentAccountId;
});
...
goApp.processSelection	= function()
{
	...
	goApp.currentAccountId	= aIDs.length == 1 ? aIDs[0] : null;
};
...
goApp.showCreateDialog	= function()
{
	...
	// clear fields
	$('#name,#reseller_id,#organization_id,#email,#info,#password').val('');
...
goApp.createOrganization = function()
{
	...
	// send request
	var oRequest	= new Request('/json/orgadmin');
	oRequest.add({
		type: 'create',
		data: oData
	});
