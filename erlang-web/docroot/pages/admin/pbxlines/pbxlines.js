goApp.setPageTitle('GTK::Lines of device');
...
goApp.addStrings({
	...
	create: 'Create',
	createAccount: 'Create new line',
	editAccount: 'Line properties',
	modifiedLost: 'Several lines that you modify were removed by '
...
goApp.addErrorMessage(goApp.errors.ERR_RECORD_LOST,
	'Line that you edit was removed by third party application.');
...
goApp.createGrid('grid', {
	...
	datatype: function(oParams)
	{
		...
		var
			oRequest	= new Request('/json/device/line'),
			oCommand	= {
				type: 'select',
				needTotalSize: true,
				needFields: !goApp.validator,
				fields: ['id', 'name', 'auth_name', 'auth_passwd', 'proxy_addr', 'proxy_port'],
				page: {
				...
				filter: {'match':'and', 'rules':[{'field':'device_id','match':'equal','value':goApp.currentDeviceId1}]}
			};
...
goApp.showCreateDialog	= function()
{
	...
	// clear fields
	$('#name,#device_id,#auth_name,#auth_passwd,#proxy_addr,#proxy_port').val('');
 	$('#device_id').val(goApp.currentDeviceId1);
...
goApp.showEditDialog	= function()
{
	...
	$('#name').val(oData.name);
	$('#device_id').val(goApp.currentDeviceId);
	$('#auth_name').val(oData.auth_name);
	$('#auth_passwd').val(oData.auth_passwd);
	$('#proxy_addr').val(oData.proxy_addr);
	$('#proxy_port').val(oData.proxy_port);
...
goApp.collectFormData	= function()
{
	...
	oData.put('name', true);
	oData.put('device_id', true);
	oData.put('auth_name', true);
	oData.put('auth_passwd', true);
	oData.put('proxy_addr', true);
	oData.put('proxy_port', true);
...
goApp.createOrganization = function()
{
	...
	// send request
	var oRequest	= new Request('/json/device/line');
	oRequest.add({
		type: 'create',
		data: oData
	});
