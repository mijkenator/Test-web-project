goApp.setPageTitle('GTK::Organizations');

/**
 * App-specific strings
 */
goApp.addStrings({
	confirmDelete: 'Delete selected organization?',
	create: 'Create',
	createAccount: 'Create new organization',
	editAccount: 'Organization properties',
	modifiedLost: 'Several organizations that you modify were removed by '
...
goApp.addErrorMessage(goApp.errors.ERR_RECORD_LOST,
	'Organization that you edit was removed by third party application.');
	...
	datatype: function(oParams)
	{
		...
		var
			...
			oCommand	= {
				...
				needTotalSize: true,
				needFields: !goApp.validator,
				fields: ['id', 'name', 'reseller_id', 'created', 'active'],
				page: {
...
goApp.showEditDialog	= function()
{
	...
	$('#name').val(oData.name);
	$('#reseller_id').val(oData.reseller_id);
...
goApp.createOrganization = function()
{
	...
	// send request
	var oRequest	= new Request('/json/organization');
	oRequest.add({
		type: 'create',
		data: oData
	});
