goApp.setPageTitle('GTK::Resellers');

/**
 * App-specific strings
 */
goApp.addStrings({
	confirmDelete: 'Delete selected reseller?',
	create: 'Create',
	createAccount: 'Create new reseller',
	editAccount: 'Reseller properties',
	modifiedLost: 'Several resellers that you modify were removed by '
...
goApp.addErrorMessage(goApp.errors.ERR_RECORD_LOST,
	'Reseller that you edit was removed by third party application.');
	...
	datatype: function(oParams)
	{
		...
		var
			...
			oCommand	= {
				...
				fields: ['id', 'name', 'email', 'password', 'info', 'created', 'active'],
				page: {
...
/**
 * Dialog to create/edit reseller
 */
...
{
	if (goApp[goApp.isUpdate ? 'updateReseller' : 'createReseller']())
		$(goApp.editorSelector).dialog('close');
...
goApp.showCreateDialog	= function()
{
	...
	// clear fields
	$('#name,#login,#email,#password,#info').val('');
...
goApp.createReseller = function()
{
	...
	// send request
	var oRequest	= new Request('/json/reseller');
	oRequest.add({
		type: 'create',
		data: oData
	});
