goApp.setPageTitle('GTK::Phone numbers');

/**
 * App-specific strings
 */
goApp.addStrings({
	confirmDelete: 'Delete selected number?',
	create: 'Create',
	createAccount: 'Create new number',
	editAccount: 'Number properties',
	modifiedLost: 'Several numbers that you modify were removed by '
...
goApp.addErrorMessage(goApp.errors.ERR_RECORD_LOST,
	'Number that you edit was removed by third party application.');
...
goApp.get_type_name_by_id = function(sValue)
{
	var ret = '7';
	switch(sValue){
		case 'extension':  ret = "Extension";   break;
		case 'did':  ret = "Did"; break;
		default: ret = "extension"
	}
	return ret
};
...
goApp.createGrid('grid', {
	...
	datatype: function(oParams)
	{
		...
		var
			oRequest	= new Request('/json/phone_number'),
			oCommand	= {
				type: 'select',
				needTotalSize: true,
				needFields: !goApp.validator,
				fields: ['id', 'number', 'account_id', 'organization_id', 'type', 'created', 'active', 'info'],
				page: {
		...
		if (goApp.searchSample != '')
			oCommand.filter	= goApp.getSearchFilter(['number']);
...
goApp.showCreateDialog	= function()
{
	...
	// clear fields
	$('#number,#account_id,#organization_id,#info,#password,#type').val('');
...
goApp.createOrganization = function()
{
	...
	// send request
	var oRequest	= new Request('/json/phone_number');
	oRequest.add({
		type: 'create',
		data: oData
	});
