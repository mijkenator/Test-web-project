...
get_users() ->
    % admins from dets, and users from mnesia
    lists:append(dets:match(users,{'$1','_','_'}), db:get_table_attribute(account, jid)).
...
authenticate(Username, Password) ->
    case db:get_first(account, fun(Account) ->
        ((Account#account.login == Username) or (Account#account.jid == Username)) and
        (Account#account.password == Password)
    end) of
        not_found -> % try to find admin
            case dets:lookup(users, Username) of
