...
console:log(["VD2", Type, Id, Values]),
...
console:log(["VD3", Values0, Values1, Definitions]),
    Fields = db:get_fields_of(Type),
console:log(["VD4", Type]),
    Errors = [{F, D} || {F, {error, {D, _}}} <- lists:zip(Fields, Values1)],
console:log(["VD5", Errors]),
...
prepare_data(Type, Action, Command) ->
    ...
    case Action of
        ...
        _ ->
            ...
            list_to_tuple([Type | [utils:to_integer(Id) | Values]])
    end.
